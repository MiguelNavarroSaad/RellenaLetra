...
public class RellenaLetra extends PApplet {
...
public void draw() {
  ...
  r_cambiaXY = sqrt((x-(x0[5]+x0[6])/2)*(x-(x0[5]+x0[6])/2)
               + (y-y0+0.75f*diametro)*(y-y0+0.75f*diametro));
  if (r_cambiaXY < diametro/2 && mousePressed) cambia = !cambia;
  if (!cambia) {
    minuscula.beginDraw();
  minuscula.pushMatrix(); minuscula.translate(0, 0); minuscula.scale(0.5f);
  ...
  minuscula.popMatrix();
    ...
    Mayuscula.beginDraw();
  Mayuscula.pushMatrix(); Mayuscula.translate(0, 0); Mayuscula.scale(0.5f);
...
public void crea(PGraphics pagina) {
  pagina.beginDraw();
...
public void boton(PGraphics pagina, String letra) {
  pagina.beginDraw();
  pagina.pushMatrix();
  pagina.translate((x0[3]+x0[6])/2, y0-0.75f*diametro);
...
public void colorea(PGraphics pagina) {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "RellenaLetra" };
    if (passedArgs != null) {
